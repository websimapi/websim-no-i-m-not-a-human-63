...
function resolveCollision(oldPos, delta) {
    const tryMove = (dx, dz) => {
        const p = oldPos.clone().add(new THREE.Vector3(dx, 0, dz));
        for (const box of barnColliders) {
            const b = box.clone(); b.expandByScalar(PLAYER_RADIUS);
...
function animate() {
    ...
    if (isMobile && isMovingToTarget && targetPosition) {
        ...
        // Replace moveRight/moveForward with collision-aware translate
...
export function initScene3D() {
    ...
    /* build barn wall colliders (ground-level) */
    (function buildBarnColliders(){
        ...
        // left side extension perimeter (matches extension position above)
